...
def run_process_tranding_scenario(code_list):
    p = Process(target=tranding_scenario, args=(code_list,))
    p.start()
    p.join()
...
def check_sell_order(code):  # 매도주문 완료 체크
...
def tranding_scenario(code_list):
    for code in code_list:
        ...
        if buy_order_cnt == 0:
            ...
            order_doc = order[0]
            mongo.insert_item({"매수주문": order_doc,
                               "code": code,
...
if __name__ == '__main__':
    ...
    print("today", today)
    scheduler.add_job(func=run_process_tranding_scenario,
