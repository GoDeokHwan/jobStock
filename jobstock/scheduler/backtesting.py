...
def run_process_trading_scenario(code_list, date):
    p = Process(target=run_tranding_scenario, args=(code_list, date))
    p.start()
    p.join()
    print("run process join")


def run_tranding_scenario(code_list, date):
    tick = 0
