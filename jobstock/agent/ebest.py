...
class EBest:
    ...
    def _execute_query(self, res, in_block_name, out_block_name, *out_fields, **set_fields):
        ...
        # LoadFromResFile : RES 파일 지정
        xa_query.LoadFromResFile(XAQuery.RES_PATH + res + ".res")

        # in_block_name 셋팅
        for key, value in set_fields.items():
            xa_query.SetFieldData(in_block_name, key, 0,
                                  value)  # 블록의 필드 데이터 설정
        ...
        # 결과 블록
        result = []
        count = xa_query.GetBlockCount(
            out_block_name)  # 블록이 Occurs 라면 Occurs 의 개수
    ...
    def get_stock_price_by_code(self, code=None, cnt="1"):
        ...
        out_params = ['date', 'open', 'high', 'low', 'close', ' sign', 'change', 'diff', 'volume', 'diff_vol', 'chdegree', 'sojinrate', 'changerate', 'fpvolume',
    ...
    def get_short_trend_by_code(self, code=None, sdate=None, edate=None):   # t1927 공매도일별추이
        ...
        out_params = ['date', 'price', 'sign', 'change', 'diff', 'volunme',
                      'value', 'gm_vo', 'gm_vo', 'gm_per', 'gm_avg', 'gm_vo_sum']
    ...
    def order_check(self, code=None, order_no=None):  # 주식 체결/미체결 조회
        ...
        out_params = ["ordno", "expcode", "medosu", "qty", "price", "cheqty", "cheprice", "ordrem", "cfmqty",
                      "status", "orgordno", "ordgb", "ordermtd", "sysprocseq", "hogagb", "price1", "orggb", "singb", "loandt"]
    ...
    def get_price_n_min_by_code(self, date, code, tick=None):  # t8412 주식차트(N분)
        in_params = {
            "shcode": code,
            "ncnt": "1",
            "qrycnt": "500",
